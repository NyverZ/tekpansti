        ...
        <div class="grid gap-8 lg:grid-cols-[1fr_0.36fr]">
            ...
            <aside class="space-y-6">
                <div class="sf-panel p-6">
                    <p class="text-sm uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Bacaan terkait</p>
                    <div class="mt-6 space-y-4">
                        @forelse ($relatedArticles as $related)
                            <a href="{{ route('articles.show', $related) }}" class="block rounded-[1.5rem] bg-slate-50 px-5 py-4">
                                <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $related->title }}</p>
                                <p class="mt-1 text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ $related->created_at->format('d M Y') }}</p>
                            </a>
                        @empty
                            <p class="text-sm text-slate-500 dark:text-slate-400">Belum ada artikel terkait.</p>
                        @endforelse
                    </div>
                </div>
                <div class="sf-panel p-6">
                    <p class="text-sm uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Lanjut belajar</p>
